...
from atomic_fetch_fucs import get_car_source_tag_by_clue_id

from atomic_fetch_price import get_car_sale_status_by_clue_id

#from atomic_fetch_fucs import get_car_source_guarantee_by_clue_id
...
def get_car_basic_info_by_clue_id_(clue_id: str,env:str="online") -> str:
    ...
    if clue_id != "-1" and clue_id and validate_clue_id(clue_id):
        ...
        task_list.extend([
                ...
                {
                    "func": get_car_sale_status_by_clue_id,
                    "args": (clue_id, env),
                    "name": "获取车源在售状态"
                },
            ])
        ...
        car_sale_status                 =   task_results.get("获取车源在售状态", {})
        #print(f"car_sale_status: {car_sale_status}")
    else:
        # 车源id不合法，无法拿到有效信息，返回空字典
        return {}
    # Step2. 解析车辆基本信息
    car_basic_info, _, _, store_id = car_basic_info_parser(car_basic_info_multi_clue_id, clue_id,battery_report,car_sale_status)
    """
       返回结果示例:
...
def get_car_sourcechannel_by_clue_id(clue_id: str,env:str="online") -> str:
    ...
    url_car_source_tag_api =     os.path.join(url_carsource, "carsourcetag/internal//carSourceTag/check")
    result_tag = get_car_source_tag_by_clue_id(clue_id,url_car_source_tag_api,key_carsource,secret_carsource)
    if result_tag == "个人车源":
        return {"车源渠道":"个人车源"}
    return {"车源渠道":result}
...
def get_car_pictures_cards_by_clue_id(clue_id: str,env:str="online") -> str:
    ...
    # 不合法的clue_id 返回空字典
    if clue_id is None or clue_id == "" or not validate_clue_id(clue_id):
        return {"car_pictures_cards_link":""}
    
    result = {"car_pictures_cards_link":""}
    # 拿到环境配置
    env_config            =     get_env_config(local_env)
    url_mall_prd          =     env_config.url_mall_prd
    key_mall_prd          =     env_config.key_mall_prd
                        ...
                        if len(carsource_images) > 0:
                            ...
                            product_details_page_link = os.path.join(url_im,
                                                                    "detail?clueId={}&hideTitlebar=1&h5Ready=1&isPageView=1".format(
                                                                        clue_id))
                            car_appearance_image_url = product_details_page_link
                            {
                            ...
                            result.update({
                            "car_pictures_cards_link": product_details_page_link,
                        })
            except Exception as e:
                logger.error(f"解析车源图片数据异常: {e}")
                return result 
...
# 第15个工具
# 获取车源所在城市
def get_car_source_city_by_clue_id_(clue_id: str,plate_city_id:str, selected_city_id:str, location_city_id:str, env:str="online") -> str:
    ...
    try:
        ext = {
            "plate_city_id": int(plate_city_id) if plate_city_id else 13,
                "selected_city_id": int(selected_city_id) if selected_city_id else 13,
                "location_city_id": int(location_city_id) if location_city_id else 13
            }
        ...
        #logger.error(f"get_car_source_city_by_clue_id_ error: {e}")
        result["car_source_city"] = {}
        return result
    return result
...
def get_car_reputation_by_clue_id_(clue_id: str,question:str, gz_user_id:str,env:str="online") -> str:
    ...
    # 拿到环境配置
    env_config                  =   get_env_config(local_env)
    url_car_reputation          =   env_config.url_car_reputation
    key_car_reputation          =   env_config.key_car_reputation
    
    try:
        car_reputation           =   get_car_reputation_by_clue_id( clue_id, question,gz_user_id, url_car_reputation, key_car_reputation)
...
def get_car_source_highlight_by_clue_id_(clue_id: str,env:str="online") -> str:
    ...
    return result
# def get_car_source_guarantee_by_clue_id_(clue_id: str,env:str="online") -> str:
#     """通过车源ID(clue_id)获取车源对应车辆原厂保修质保政策"""
...
#     url_zhibao             =     env_config.url_zhibao
#     key_zhibao             =     env_config.key_zhibao
#     secret_zhibao          =     env_config.secret_zhibao

#     try:
#         car_zhibao_policy        =   get_car_source_guarantee_by_clue_id( clue_id, url_zhibao,  key_zhibao, secret_zhibao)
#         result["车源原厂保修质保政策"] =   car_zhibao_policy
#         return result
#     except Exception as e:
#         #logger.error(f"get_car_reputation_by_clue_id_ error: {e}")
#         return result
...
if __name__ == "__main__":
    ...
    # result = get_car_reputation_by_clue_id_("123", "这车怎么样？", "174132952062000095""online")
    ...
    # result = get_car_sourcechannel_by_clue_id("164651578","online") 
    # print(f"clue_id: 164651578, result: {result}")

    # result = get_car_sourcechannel_by_clue_id("164511422","online") 
    # print(f"clue_id: 164511422, result: {result}")

    # result = get_car_sourcechannel_by_clue_id("164541196","online") 
    # print(f"clue_id: 164541196, result: {result}")

    # result = get_car_sourcechannel_by_clue_id("163835919","online") 
    # print(f"clue_id: 163835919, result: {result}")


    # result = get_car_sourcechannel_by_clue_id("164654177","online") 
    # print(f"clue_id: 164654177, result: {result}")

    # result = get_car_sourcechannel_by_clue_id("164077892","online") 
    # print(f"clue_id: 164077892, result: {result}")

    # result = get_car_source_guarantee_by_clue_id_("160179652","online") 
    # print(f"clue_id: 160179652, result: {result}")
