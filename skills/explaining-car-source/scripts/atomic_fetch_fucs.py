...
@log_http_timing
def get_car_zhibao_policy_by_clue_id(clue_id, url, gz_user_id, key_zhibao, secret_zhibao):
    """根据车源id，获取车源对应车辆原厂保修质保政策"""
    headers = {"Content-Type": "application/json"}
    params = {"clueId": clue_id}
    all_params = create_params(params, key_zhibao, secret_zhibao)
    try:
        response = requests.get(url, params=all_params, headers=headers)
        return parse_response_data(response)
    except Exception as e:
        logger.error(f"get_car_zhibao_policy_by_clue_id error: {e}")
        return ""

@log_http_timing
def get_car_source_tag_by_clue_id(clue_id, url, CARSOURCE_APP_KEY, CARSOURCE_APP_SECRET):
    """瑕疵车/个人车源判断"""
    tags = []
    for tag_id, tag_name in [("1393", "瑕疵车"), ("1368", "个人车源")]:
        params = {"clue_id": clue_id, "tag": tag_id}
        all_params = create_params(params, CARSOURCE_APP_KEY, CARSOURCE_APP_SECRET)
