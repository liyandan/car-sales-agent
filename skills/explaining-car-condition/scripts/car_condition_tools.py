...
from atomic_fetch_fucs import get_car_paint_tag_by_clue_id
...
def get_car_general_knowledge_by_clue_id_(question:str,gz_user_id:str,env:str="online") -> str:
    ...
    try:
        car_knowledge = get_car_general_knowledge_without_clue_id( question, gz_user_id, url_car_general_knowledge, key_car_general_knowledge )
    ...
    return result
# 获取车源对应车辆的基本信息
def get_car_carpaintstatus_by_clue_id(clue_id: str,env:str="online") -> str:
    """通过车源ID(clue_id)获取漆面状态信息（如：车商车、个人车等）"""
    loca_env        =    "online"
    if env is None or env == "" or env not in ( "online","test","preview","pre"):
        local_env   =    "online"
    else:
    ...
    result_tag = get_car_paint_tag_by_clue_id(clue_id,url_car_source_tag_api,key_carsource,secret_carsource)
    if result_tag == "原版原漆":
        return {"漆面状态":"原版原漆"}
    return {"漆面状态":"非原版原漆"}
...
if __name__ == "__main__":
    ...
    result = get_car_carpaintstatus_by_clue_id( "165658840","test") 
    print(f"clue_id: 163993382, result: {result}")

    result = get_car_carpaintstatus_by_clue_id( "165670108","test") 
    print(f"clue_id: 165670108, result: {result}")

    result = get_car_carpaintstatus_by_clue_id( "164175532","test") 
    print(f"clue_id: 164175532, result: {result}")

    result = get_car_carpaintstatus_by_clue_id( "164862773","test") 
    print(f"clue_id: 164862773, result: {result}")
    print("Finished!")
